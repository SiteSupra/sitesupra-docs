...
SiteSupra security layer is heavily based on `Symfony security component <https://github.com/symfony/Security>`_.
...
Default security configuration is stored in ``Supra\Package\CmsAuthentication\Resources\config\config.yml``. Apart
of paths and services, it defines a shared user source (explained below), sets up user providers (bound to
...
While developing web project it is nice to have a shared user database with some default user accounts in there or share users between
